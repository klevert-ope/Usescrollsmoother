 ...
 * @property {number} smooth - the time (in seconds) that it takes to "catch up" to the native scroll position.
 * Default is 1.
 * @property {boolean,number} smoothTouch - by default, ScrollSmoother will NOT apply scroll smoothing on touch-only
 ...
 *   Default is false.
 * @property {boolean} normalizeScroll - if true, it forces scrolling to be done on the JavaScript thread,
 * ensuring it is synchronized and the address bar doesn't show/hide on mobile devices.
 * This is the same as calling ScrollTrigger.normalizeScroll()
 * except that it debounces because smooth scrolling makes that possible.
 * Default is false.
 * @property {boolean} ignoreMobileResize - if true,
 ...
 * Default is false.
 * @property {string,boolean,array} effects - if true, ScrollSmoother will find all elements that have a data-speed
 *   and/or data-lag attribute and apply those effects accordingly so that they move at the designated speed or delay,
 *   so data-speed="0.5" would scroll at half the normal speed, and data-speed="2" would scroll at twice the normal
 *   speed.
 *   data-lag="0.8" would take 0.8 seconds to "catch up" to the smoothed scroll position.
 *   You can also use
 *   selector text or an Array of elements, so effects: ".box" would only look for the attributes on elements with the
 *   ".box" class.
 *   You can use the effects() method to apply effects directly via JavaScript instead.
 *   See that method's
 *   docs for more details about how effects work.
 *   Note: effects should not be nested.
 *   Default is false.
 * @property {string} effectsPrefix - perhaps you're already using data-speed and/or data-lag for other purposes,
 ...
 * @property {number} effectsPadding - Normally effects applied to a particular element begin as soon as the natural
 *   position of the element enters the viewport and then end when the natural position leaves the viewport, but in
 ...
 * @property {function} onUpdate - a function to call after each time the SmoothScroller updates the position of the
 *   content.
 * @property {function} onFocusIn - a function to call when a new element receives focus,
 ...
 * @property {function} onStop - a function to call when the smoothed scroll comes to a stop (catches up to the native
 *   scroll position).
 */

/**
 * @typedef {Object} UseScrollSmootherProps
 * @property {ScrollSmootherConfig} [config] - The configuration object.
 * If not provided,
 ...
 */
function useScrollSmoother(props = {}) {
  const {
    config = {
      smooth: 1,
      smoothTouch: false,
  ...
  UseIsomorphicLayoutEffect(() => {
    if (!smoothWrapperRef.current || !smoothContentRef.current) {
      console.warn('Smooth wrapper or content Ref not found. Make sure the refs exist.');
      return;
    }
    const gsapContext = gsap.context(() => {
      let configuration = gsap.matchMedia();
      configuration.add('(max-width: 900px)', () => {
